...
function inferAuthRefreshScope(
  ...
  if (
    ...
    path === "/customer-auth/google/login" ||
    path === "/customer-auth/google/register" ||
...
async function refreshAuthSession(scope: Exclude<AuthRefreshScope, null>) {
  if (scope === "admin") {
    if (!adminRefreshPromise) {
      adminRefreshPromise = refreshAuthSessionOnce("/auth/refresh").finally(
        () => {
...
function calculateProductPromoPrice(product: Product, price: number) {
  ...
  if (!isDateRangeActive(product.discountStartDate, product.discountEndDate)) {
    return null;
  }
...
function calculateCollectionPromoPrice(product: Product, price: number) {
  ...
  if (!isDateRangeActive(collection.promoStartDate, collection.promoEndDate)) {
    return null;
  }
...
function normalizeProduct(product: Product): Product {
  ...
  if (collectionPromoPrice !== null) {
    return {
      ...
      discountPercentage: calculateCollectionPromoPercentage(normalizedProduct),
    };
  }
...
export async function loginCustomerWithGoogle(
  payload: GoogleCustomerLoginPayload,
) {
  return fetchJson<CustomerAuthResponse>("/customer-auth/google/login", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}

export async function registerCustomerWithGoogle(
  payload: GoogleCustomerRegisterPayload,
) {
  return fetchJson<CustomerAuthResponse>("/customer-auth/google/register", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
...
export async function changeCustomerPassword(
  ...
  return fetchJson<ChangeCustomerPasswordResponse>("/customer-auth/password", {
    method: "PATCH",
    headers: buildAuthHeaders(token),
    body: JSON.stringify(payload),
  });
...
export async function revokeOtherAdminSessions() {
  return fetchJson<RevokeSessionsResponse>("/auth/sessions/revoke-others", {
    method: "POST",
  });
}
...
export async function requestAdminPasswordReset(email: string) {
  return fetchJson<PasswordResetRequestResponse>("/auth/forgot-password", {
    method: "POST",
...
export async function resetCustomerPassword(payload: ResetPasswordPayload) {
  return fetchJson<{ message: string }>("/customer-auth/reset-password", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
